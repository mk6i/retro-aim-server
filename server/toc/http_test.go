...
func TestOSCARProxy_NewServeMux(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:           "Retrieve profile with improperly escaped `cookie` query param",
			path:           "/info?from=me&user=them&cookie=" + cookie + "%25",
			expectedStatus: http.StatusBadRequest,
			expectedBody:   "invalid auth cookie",
		},
		{
			name:           "Retrieve profile with improperly base64-encoded `cookie` query param",
			path:           "/info?from=me&user=them&cookie=zzz" + cookie,
			expectedStatus: http.StatusBadRequest,
			expectedBody:   "invalid auth cookie",
		},
		{
