...
func (c *ChatRegistry) SessionIDs() []int {
	c.m.RLock()
	defer c.m.RUnlock()
...
func (s OSCARProxy) Signout(ctx context.Context, me *state.Session, chatRegistry *ChatRegistry) {
	...
	for _, sessID := range chatRegistry.SessionIDs() {
		chatSess := chatRegistry.RetrieveSess(sessID)
		if chatSess == nil {
			continue
		}
		s.AuthService.SignoutChat(ctx, chatSess)
		chatSess.Close() // stop async server SNAC reply handler for this chat room
	}
}
