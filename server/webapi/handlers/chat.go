...
func (h *ChatHandler) CreateAndJoinChat(w http.ResponseWriter, r *http.Request) {
	...
	chatSession, room, err := h.ChatManager.CreateAndJoinChat(aimsid, roomID, roomName, string(session.ScreenName))
	if err != nil {
...
func (h *ChatHandler) SendMessage(w http.ResponseWriter, r *http.Request) {
	...
	// Validate session
	_, err := h.SessionManager.GetSession(aimsid)
	if err != nil {
		h.Logger.Error("invalid session", "aimsid", aimsid, "error", err)
		SendError(w, http.StatusUnauthorized, "Authentication Required")
		return
	...
	// Send the message
	err = h.ChatManager.SendMessage(chatsid, message, whisperTarget)
	if err != nil {
...
func (h *ChatHandler) SetTyping(w http.ResponseWriter, r *http.Request) {
	...
	// Set typing status
	err = h.ChatManager.SetTyping(chatsid, typingStatus)
	if err != nil {
...
func (h *ChatHandler) LeaveChat(w http.ResponseWriter, r *http.Request) {
	...
	// Leave the chat room
	err = h.ChatManager.LeaveChat(chatsid)
	if err != nil {
