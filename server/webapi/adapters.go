...
func ICBMToWebAPIEvent(icbm wire.SNAC_0x04_0x07_ICBMChannelMsgToClient) (state.WebAPIEvent, error) {
	...
	// Create WebAPI event
	event := state.WebAPIEvent{
		Type:      state.EventTypeIM,
...
func TypingNotificationToWebAPIEvent(notification wire.SNAC_0x04_0x14_ICBMClientEvent) state.WebAPIEvent {
	typing := false
	...
	return state.WebAPIEvent{
		Type:      state.EventTypeTyping,
		Timestamp: time.Now().Unix(),
		Data: state.TypingEvent{
...
func PresenceUpdateToWebAPIEvent(screenName string, online bool, awayMsg string, statusBitmask uint32) state.WebAPIEvent {
	stateStr := "offline"
	if online {
		stateStr = "online"
	...
	return state.WebAPIEvent{
		Type:      state.EventTypePresence,
		Timestamp: time.Now().Unix(),
		Data: state.PresenceEvent{
			AimID:   screenName,
			State:   stateStr,
			AwayMsg: awayMsg,
		},
