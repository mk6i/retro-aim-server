...
func NewOSCARBridgeStore(db *sql.DB) *OSCARBridgeStore {
	return &OSCARBridgeStore{db: db}
}
...
func (s *OSCARBridgeStore) GetBridgeSession(ctx context.Context, webSessionID string) (*OSCARBridgeSession, error) {
	...
	err := s.db.QueryRowContext(ctx, query, webSessionID).Scan(
	...
	go s.touchSession(webSessionID)
...
func (s *OSCARBridgeStore) GetBridgeSessionByScreenName(ctx context.Context, screenName string) ([]*OSCARBridgeSession, error) {
	...
	rows, err := s.db.QueryContext(ctx, query, screenName)
	if err != nil {
...
func (s *OSCARBridgeStore) DeleteBridgeSession(ctx context.Context, webSessionID string) error {
	...
	result, err := s.db.ExecContext(ctx, query, webSessionID)
	if err != nil {
...
func (s *OSCARBridgeStore) touchSession(webSessionID string) {
	query := `UPDATE oscar_bridge_sessions SET last_accessed = ? WHERE web_session_id = ?`
	s.db.Exec(query, time.Now(), webSessionID)
}
...
func (s *OSCARBridgeStore) ValidateOSCARCookie(ctx context.Context, cookie []byte) (*OSCARBridgeSession, error) {
	...
	err := s.db.QueryRowContext(ctx, query, cookieHex).Scan(
		&session.WebSessionID,
		&session.OSCARCookie,
		&session.BOSHost,
		&session.BOSPort,
	...
	// Update last accessed time
	go s.touchSession(session.WebSessionID)
