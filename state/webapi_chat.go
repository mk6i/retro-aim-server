...
func NewWebAPIChatManager(db *sql.DB, logger *slog.Logger, sessions *WebAPISessionManager) *WebAPIChatManager {
...
func (m *WebAPIChatManager) CreateAndJoinChat(aimsid, roomID, roomName, screenName string) (*ChatSession, *WebAPIChatRoom, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if roomID != "" {
		...
		room, err = m.getRoomByName(roomName)
		if err != nil && !errors.Is(err, sql.ErrNoRows) {
			return nil, nil, fmt.Errorf("failed to get room by name: %w", err)
		}
		// If room doesn't exist, create it
		if room == nil {
			room, err = m.createRoom(roomName, screenName)
			if err != nil {
	...
	existingSession, _ := m.getUserSessionInRoom(aimsid, room.RoomID)
	if existingSession != nil {
		return existingSession, room, nil
	}

	// Check room capacity
	count, err := m.getParticipantCount(room.RoomID)
	if err != nil {
	...
	participants, _ := m.getParticipants(room.RoomID)
...
func (m *WebAPIChatManager) SendMessage(chatsid, message, whisperTarget string) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	// Get session
	session, err := m.getSessionByChatSID(chatsid)
	if err != nil {
	...
	if whisperTarget != "" {
		...
		targetSession, _ := m.getUserSessionInRoomByScreenName(session.RoomID, whisperTarget)
...
func (m *WebAPIChatManager) SetTyping(chatsid, typingStatus string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *WebAPIChatManager) LeaveChat(chatsid string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	count, _ := m.getParticipantCount(session.RoomID)
	if count == 0 {
		m.closeRoom(session.RoomID)
	}
...
func (m *WebAPIChatManager) getRoomByID(roomID string) (*WebAPIChatRoom, error) {
	var room WebAPIChatRoom
	err := m.db.QueryRow(`
...
func (m *WebAPIChatManager) getRoomByName(roomName string) (*WebAPIChatRoom, error) {
	var room WebAPIChatRoom
	err := m.db.QueryRow(`
		SELECT room_id, room_name, description, room_type, category_id, 
		       creator_screen_name, created_at, closed_at, max_participants
...
func (m *WebAPIChatManager) createRoom(roomName, creatorScreenName string) (*WebAPIChatRoom, error) {
	room := &WebAPIChatRoom{
...
func (m *WebAPIChatManager) getSessionByChatSID(chatsid string) (*ChatSession, error) {
...
func (m *WebAPIChatManager) getUserSessionInRoom(aimsid, roomID string) (*ChatSession, error) {
	var session ChatSession
	err := m.db.QueryRow(`
		SELECT chat_sid, aimsid, room_id, screen_name, instance_id, joined_at, left_at
		FROM web_chat_sessions
...
func (m *WebAPIChatManager) getUserSessionInRoomByScreenName(roomID, screenName string) (*ChatSession, error) {
	var session ChatSession
	err := m.db.QueryRow(`
		SELECT chat_sid, aimsid, room_id, screen_name, instance_id, joined_at, left_at
		FROM web_chat_sessions
...
func (m *WebAPIChatManager) getParticipantCount(roomID string) (int, error) {
	var count int
	err := m.db.QueryRow(`
...
func (m *WebAPIChatManager) getParticipants(roomID string) ([]string, error) {
	rows, err := m.db.Query(`
...
func (m *WebAPIChatManager) closeRoom(roomID string) {
	now := time.Now().Unix()
	m.db.Exec(`
...
func (m *WebAPIChatManager) sendChatEventToUser(aimsid string, event ChatEventData) {
	...
	session, err := m.sessions.GetSession(aimsid)
	if err != nil {
...
func (m *WebAPIChatManager) GetRecentMessages(roomID string, limit int) ([]*ChatMessage, error) {
	rows, err := m.db.Query(`
...
func (m *WebAPIChatManager) CleanupInactiveSessions() {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	for rows.Next() {
		...
		// Mark as left
		now := time.Now().Unix()
		m.db.Exec(`UPDATE web_chat_sessions SET left_at = ? WHERE chat_sid = ?`, now, chatsid)
		m.db.Exec(`DELETE FROM web_chat_participants WHERE room_id = ? AND screen_name = ?`, 
			roomID, screenName)
