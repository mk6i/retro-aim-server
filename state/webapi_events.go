...
// WebAPIEventType defines the type of WebAPI event.
type WebAPIEventType string
...
const (
	...
	EventTypeSessionEnded WebAPIEventType = "sessionEnded"
	EventTypeRateLimit    WebAPIEventType = "rateLimit"
)

// WebAPIEvent represents an event to be delivered to a web client.
type WebAPIEvent struct {
	Type      WebAPIEventType `json:"type"`
...
func (q *EventQueue) Push(eventType WebAPIEventType, data interface{}) {
	q.closedMu.RLock()
	if q.closed {
		q.closedMu.RUnlock()
		return
...
func (q *EventQueue) Fetch(ctx context.Context, lastSeqNum uint64, timeout time.Duration) ([]WebAPIEvent, error) {
	q.closedMu.RLock()
	if q.closed {
		q.closedMu.RUnlock()
...
func (q *EventQueue) getEventsAfter(seqNum uint64) []WebAPIEvent {
	var result []WebAPIEvent
...
func (q *EventQueue) GetAllEvents() []WebAPIEvent {
	q.mu.RLock()
	defer q.mu.RUnlock()

	result := make([]WebAPIEvent, len(q.events))
