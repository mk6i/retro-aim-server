...
// WebAPIBuddyListManager handles the conversion of OSCAR feedbag data
// to WebAPI buddy list format for web clients.
type WebAPIBuddyListManager struct {
	db               *sql.DB
...
// NewWebAPIBuddyListManager creates a new instance of the buddy list manager.
func NewWebAPIBuddyListManager(db *sql.DB, feedbagRetriever FeedbagRetriever, sessionRetriever SessionRetriever, logger *slog.Logger) *WebAPIBuddyListManager {
	return &WebAPIBuddyListManager{
		db:               db,
		feedbagRetriever: feedbagRetriever,
		sessionRetriever: sessionRetriever,
		logger:           logger,
...
type FeedbagRetriever interface {
	RetrieveFeedbag(ctx context.Context, screenName IdentScreenName) ([]wire.FeedbagItem, error)
}

// SessionRetriever provides methods to retrieve session information.
...
func (m *WebAPIBuddyListManager) GetBuddyListForUser(ctx context.Context, screenName IdentScreenName) ([]WebAPIBuddyGroup, error) {
...
func (m *WebAPIBuddyListManager) getBuddyInfo(buddyName string) WebAPIBuddyInfo {
	// Default to offline
	...
	buddyScreenName := NewIdentScreenName(buddyName)
...
func (m *WebAPIBuddyListManager) GetPresenceForBuddy(screenName string) WebAPIBuddyInfo {
	return m.getBuddyInfo(screenName)
}

// GetOnlineBuddies returns a list of all online buddies for a user.
func (m *WebAPIBuddyListManager) GetOnlineBuddies(ctx context.Context, userScreenName IdentScreenName) ([]WebAPIBuddyInfo, error) {
...
func (m *WebAPIBuddyListManager) FormatBuddyListEvent(groups []WebAPIBuddyGroup) map[string]interface{} {
