...
func NewWebAPITokenStore(db *sql.DB) *WebAPITokenStore {
	return &WebAPITokenStore{db: db}
}

// StoreToken saves an authentication token for a user.
func (s *WebAPITokenStore) StoreToken(token string, screenName IdentScreenName, expiresAt time.Time) error {
	query := `
	...
	`
	_, err := s.db.Exec(query, token, screenName.String(), expiresAt, time.Now())
	if err != nil {
...
func (s *WebAPITokenStore) ValidateToken(token string) (IdentScreenName, error) {
	...
	query := `
		...
		WHERE token = ?
	`
	err := s.db.QueryRow(query, token).Scan(&screenNameStr, &expiresAt)
	if err != nil {
...
func (s *WebAPITokenStore) DeleteToken(token string) error {
...
func (s *WebAPITokenStore) CleanupExpiredTokens() error {
...
func (u *SQLiteUserStore) AuthenticateUser(username, password string) (*User, error) {
...
func (u *SQLiteUserStore) FindUserByScreenName(screenName IdentScreenName) (*User, error) {
	ctx := context.Background()
