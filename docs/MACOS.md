...
4. **Configure Server Address**
   ...
   Set the default listener in `OSCAR_ADVERTISED_LISTENERS` in `settings.env` to a hostname and port that the AIM
...
6. **Test**
   ...
   Configure the AIM client to connect to the host and port from `OSCAR_ADVERTISED_LISTENERS` in `settings.env`. If using the default server setting, set host to `127.0.0.1` and port `5190`.
