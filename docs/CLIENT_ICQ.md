...
3. **Configure OSCAR Host**
    ...
    - Set `Value data` to the hostname from `OSCAR_ADVERTISED_LISTENERS` found in Retro AIM Server
      configuration `config/settings.env`. For example, if `OSCAR_ADVERTISED_LISTENERS=LOCAL://127.0.0.1:5190`, use
      `127.0.0.1`.
    - Click OK.
...
4. **Configure Server Port (uncommon)**
   ...
   ports.
    ...
    - Set `Value data` to the port number from `OSCAR_ADVERTISED_LISTENERS` found in Retro AIM Server configuration
      `config/settings.env`. For example, if `OSCAR_ADVERTISED_LISTENERS=LOCAL://127.0.0.1:5190`, use `5190`.
    - Click OK.
